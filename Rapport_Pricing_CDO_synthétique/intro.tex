...
\subsubsection{Espace probabilisé et filtration}\label{subsubsec:espace_prob_filtre}
...
\noindent Cette fois, nous définissons l'équation différentielle stochatique suivante :
\begin{equation}
    \begin{cases}
        B_0 = 1 \\
        dB_t = r_t B_t dt
...
Néanmoins, la loi marginale de $r_t$ est connue explicitement. $r_t$ suit une loi du $\chi^2$ non centrée (après renormalisation). Plus précisément, pour tout $t>0$, la distribution conditionnelle de $r_t$ sachant $r_0$ est une loi du $\chi^2$ non centrale.

Cette propriété permet notamment une simulation exacte du processus, sans schéma d’Euler.

\medskip

\textbf{Conclusion et ouverture vers d’autres modèles}

Les modèles de Vasicek et CIR permettent tous deux d'obtenir des formules semi-explicites pour les obligations zéro-coupon, ce qui en fait des modèles de référence malgré leurs hypothèses simplificatrices.
Leurs paramètres constants les empêchent cependant de calibrer exactement une courbe de taux observée. Il s'agit d'une limitation, que corrigent des modèles plus complexes comme Hull--White ou G2++, au prix d'une implémentation plus lourde.
Ils restent néanmoins des bases solides pour comprendre la simulation de taux court et les méthodes numériques associées.
...
\begin{equation}
    \Pi_X(t,T) = B_t\, \mathbb{E}^{\mathbb{P}^*}\!\left[ \frac{X}{B_T} \,\middle|\, \mathcal{F}_t \right].
\end{equation}
...
\begin{equation}
    D(t,T) = B_t\, \mathbb{E}^{\mathbb{P}^*}\!\left[ \frac{1}{B_T} \,\middle|\, \mathcal{F}_t \right]
\end{equation}
...
risques entrent en jeu : l'instant de défaut et la perte en cas de défaut (\emph{Loss Given Default}).
