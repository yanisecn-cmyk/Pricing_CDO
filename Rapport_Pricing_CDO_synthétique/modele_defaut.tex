...
Dans l'introduction (cf. section \ref{subsubsec:espace_prob_filtre}) nous avons défini le
processus de défaut $H$ indiquant la présence d'un défaut sur un contrat à un temps donné.
Cependant nous pouvons nous demander quelle dynamique suit ce processus afin d'éventuellement
retrouver la loi de la variable aléatoire de temps de défaut $\tau$. Enfin dans le cas de plusieurs
contrats nous verrons dans cette section comment retrouver la loi conjointe des temps de défauts
à partir des lois marginales.
...
qui assure l'existence d'une décomposition unique du processus de défaut intégrable et $\mathbb{G}$-adapté $H$ défini section
\ref{subsubsec:espace_prob_filtre} de la forme $H_t = M_t + A_t$ où $M_t$ est une
...
\subsection{Modélisation du risque d'un CDS}\label{subsec:modele_risque_CDS}

La détermination du prix d'un Credit Default Swap repose sur la modélisation 
probabiliste du temps de défaut de l'entité de référence. En nous basant sur les probabilités
établies pour les modèles à intensité dans la section précédente nous pouvons établir les
valeurs des jambes fixe et de protection et ainsi calculer le spread équitable pour le contrat.
...
\subsubsection{Modèles de copules}\label{subsubsec:modele_copule}

La valorisation des tranches de CDO synthétiques repose sur la modélisation de la distribution
jointe des temps de défaut $\tau_1, \tau_2, \ldots, \tau_n$ du portefeuille. Pour chaque entité $i$,
la distribution marginale $F_i(t) = \mathbb{P}^*(\tau_i \leq t)$ est supposée connue, typiquement par
calibration sur les spreads de CDS individuels comme exposé en section~3. La difficulté consiste
alors à spécifier la structure de dépendance entre ces marginales sans la confondre avec leur
comportement individuel.

Le théorème de Sklar fournit le cadre théorique adapté à cette séparation :
il stipule qu'il existe une fonction de copule $C : [0,1]^n \rightarrow [0,1]$ telle que
\begin{equation}
    F(\tau_1, \tau_2, \ldots, \tau_n) = C\bigl(F_1(\tau_1), F_2(\tau_2), \ldots, F_n(\tau_n)\bigr).
\end{equation}
La copule $C$ encode entièrement la structure de dépendance, indépendamment des lois marginales.
Elle est de plus unique lorsque les marginales sont continues. La modélisation des défauts corrélés
se ramène ainsi au choix d'une copule paramétrée, les marginales étant fixées par le marché des CDS.
...
dépendance de queue. Une approche alternative dite \emph{bottom-up} ou simplement à intensité stochastique
décompose l'intensité de défaut de chaque entité $i$ en
\begin{equation}
